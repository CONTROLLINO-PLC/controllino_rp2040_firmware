...
wsen_error_code_t wsen_temp_init(wsen_temp_t* dev, wsen_temp_cfg_t* cfg)
{
    ...
    // Init hardware reset and wsen_error_code_t pins 
    ...
    // Check coms
    uint8_t id;
    wsen_error_code_t ret = wsen_temp_get_id(dev, &id);
...
{
    uint16_t raw_temp;
    wsen_error_code_t ret = wsen_temp_get_raw(dev, &raw_temp);
    if (ret != WSEN_TEMP_OK)
        return ret;
