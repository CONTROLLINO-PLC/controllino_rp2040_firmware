...
wsen_temp_error_code_t wsen_temp_init(wsen_temp_t* dev, wsen_temp_cfg_t* cfg)
{
    ...
    if (platform_i2c_init(cfg->i2c, cfg->i2c_speed, cfg->sda_pin, cfg->scl_pin) != WSEN_TEMP_OK)
        return WSEN_TEMP_INIT_ERROR;
    // Init hardware reset and wsen_temp_error_code_t pins 
    ...
    // Check coms
    uint8_t id;
    wsen_temp_error_code_t ret = wsen_temp_get_id(dev, &id);
...
{
    uint16_t raw_temp;
    wsen_temp_error_code_t ret = wsen_temp_get_raw(dev, &raw_temp);
    if (ret != WSEN_TEMP_OK)
        return ret;
