...
extern uint32_t TEST_MCP356X_ADC_DATA;
...
platform_err_code_t mock_spi_transfer(uint8_t* txdata, uint8_t* rxdata, size_t len)
{
    ...
    else if (fcmd_addr == MCP356X_REG_ADC_DATA) {
        if (rw_cmd == MCP356X_CMD_INC_READ)
        {
            *(rxdata + 1) = (TEST_MCP356X_ADC_DATA >> 16) & 0xFF;
            *(rxdata + 2) = (TEST_MCP356X_ADC_DATA >> 8) & 0xFF;
            *(rxdata + 3) = TEST_MCP356X_ADC_DATA & 0xFF;
        }
    }
