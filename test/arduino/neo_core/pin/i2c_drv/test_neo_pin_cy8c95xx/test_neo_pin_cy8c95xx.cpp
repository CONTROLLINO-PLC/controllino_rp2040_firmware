...
{
    TEST_ASSERT_EQUAL(CY8C95XX_GPIO_6, _NEO_CORE_DO7->getPin());
    TEST_ASSERT_EQUAL(ControllinoNeoPin::CY8C95XX_PIN, _NEO_CORE_DO7->getType());
}
 
void test_cy8c95xx_pin_pinMode_is_input_pull_up_down_or_output()
{
    TEST_ASSERT_EQUAL(INPUT_PULLUP, _NEO_CORE_DO7->getMode());
    pinMode(NEO_CORE_DO7, INPUT_PULLUP);
    TEST_ASSERT_EQUAL(INPUT_PULLUP, _NEO_CORE_DO7->getMode());
    pinMode(NEO_CORE_DO7, INPUT_PULLDOWN);
    TEST_ASSERT_EQUAL(INPUT_PULLDOWN, _NEO_CORE_DO7->getMode());
    pinMode(NEO_CORE_DO7, OUTPUT);
    TEST_ASSERT_EQUAL(OUTPUT, _NEO_CORE_DO7->getMode());
...
void test_cy8c95xx_pin_analogWrite()
{
    ...
    cy8c95xx_read_bit(neo_cy8c95xx, CY8C95XX_REG_SEL_PWM_OUT, (_NEO_CORE_DO7->getPin() % 8), &res_pwm_en);
...
{
    setDigitalThreshold(_NEO_CORE_DO7, 0);
    TEST_ASSERT_EQUAL(~0U, getDigitalThreshold(_NEO_CORE_DO7));
}
