...
void test_mcp23s17_generic_transfer_ok()
{
    ...
    ret = mcp23s17_generic_transfer(&dev, MCP23S17_ADDRESS_0, MCP23S17_IODIRA_BANK0, MCP23S17_CMD_READ, tx_data, &rx_data);
    TEST_ASSERT_EQUAL(PLATFORM_OK, ret);
    TEST_ASSERT_EQUAL(0xFF, rx_data);
    // default value of IPOLA register is 0x00
    ret = mcp23s17_generic_transfer(&dev, MCP23S17_ADDRESS_0, MCP23S17_IPOLA_BANK0, MCP23S17_CMD_READ, tx_data, &rx_data);
    TEST_ASSERT_EQUAL(PLATFORM_OK, ret);
    ...
    ret = mcp23s17_generic_transfer(&dev, MCP23S17_ADDRESS_0, MCP23S17_IOCON_BANK0, MCP23S17_CMD_WRITE, tx_data, &rx_data);
    TEST_ASSERT_EQUAL(PLATFORM_OK, ret);
    // read IOCON register
    ret = mcp23s17_generic_transfer(&dev, MCP23S17_ADDRESS_0, MCP23S17_IOCON_BANK0, MCP23S17_CMD_READ, tx_data, &rx_data);
    TEST_ASSERT_EQUAL(PLATFORM_OK, ret);
